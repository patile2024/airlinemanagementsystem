...
const Invoice = () => {
  ...
  const loadData = async () => {
    const Returnresponse = await Axios.get(
      "http://localhost:5001/invoicefares"
    );
  ...
  useEffect(() => {
    ...
    Axios.post("http://localhost:5001/UpdateFlightBooking",{
        id:initialState.sc_id,
    })
    Axios.get(`http://localhost:5001/invoice/${initialState.cl_id}`).then((resp) =>
      setData({ ...resp.data[0] })
    );
    loadData();
  }, []);
  const foo = async () => {
    await Axios.post("http://localhost:5001/invoiceconfirm", {
      id: initialState.sc_id,
      departure: user.departure,
    });
    await Axios.post("http://localhost:5001/invoiceconfirmAgain", {
      id: initialState.cl_id,
      flight_no: user.flight_no,
      fares: user.price.substr(2, 6),
    });
    Axios.delete('http://localhost:5001/removeSearch');
